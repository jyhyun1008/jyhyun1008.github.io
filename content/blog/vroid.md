...
머리카락 텍스처로서는 같은 텍스처를 반복해서 사용하는 것보다 그 위치에 맞는 고유 텍스처를 사용해 주는 게 도움이 됩니다. 저는 모든 텍스처를 따로 그리지는 않고 하이라이트가 있는 버전과 없는 버전 두 개의 텍스처만을 사용했는데 하이라이트의 위치와 너비 등을 텍스처의 Width, Highlights Position, Offset으로 조정할 수 있습니다. 앞머리에 하이라이트를 넣을 때 사용했는데, 앞머리 전체에 하이라이트를 넣듯 크게 그려준 다음에 이 텍스처 파라미터를 조정해주면 된답니다.

![](/blog/vroid-5.jpg)

| ![](/blog/vroid-6.png) | ![](/blog/vroid-7.png) |
|---|---|
| 하이라이트 넓이 맞추기 | 하이라이트 높이 맞추기 |
