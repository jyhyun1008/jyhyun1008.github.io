...
하지만 저는 이번에도 역시 브이로이드로 새 모델을 제작하기로 했습니다. 라이브 2D라는 포맷은 확실히 악기가 나름 중심이 되는 제 방송에 맞지 않았는데, 역시 3D 모델을 처음부터 만들기 위해 외주를 넣는 것보다는 제가 혼자 만드는 게 마음이 편했달까요.
...
사실 이미 의상 디자인도 다 끝내 놓았는데, 아직 제작은 하지 않아서 이정도만 첨부해 두겠습니다. 기존 모델이 3D라는 느낌이 많이 든다면, 새 모델이 조금 더 2D 그림에 가까운 것을 볼 수 있습니다.
...
머리카락 텍스처로서는 같은 텍스처를 반복해서 사용하는 것보다 그 위치에 맞는 고유 텍스처를 사용해 주는 게 도움이 됩니다. 물론 저 역시 두 개의 텍스처를 돌려가며 사용했는데 그것은 텍스처의 width, offset으로 조정할 수 있습니다.

텍스처를 만지는 김에 아웃라인 색상도 만져주세요. 짙은 머리카락을 사용하는 것이 아니라면 검정색은 너무 부자연스럽고 3D 티가 납니다. 아웃라인은 나중에 (어딘가) 에서 두께를 조정할 수 있습니다.

제가 사용한 두 개의 텍스처는 사실 같은 텍스처지만, 하이라이트가 있는 버전과 없는 버전으로 나뉘어집니다. 하이라이트가 없는 버전은 텍스처를 그다지 조정할 필요가 없습니다. 물론 위아래 그라데이션은 있기 떄문에 이 위치는 맞춰줘야겠죠. 

주로 앞머리에 사용되는 하이라이트가 있는 버전은 우선 임시로 무지개색 그라데이션이 있는 텍스처를 이용해 위치를 먼저 잡게 됩니다.